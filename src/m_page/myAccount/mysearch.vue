<template>
 ...
 <div class="mysearch-main">
	...
	<el-tab-pane label="扫码记录" name="fourth"> 
	<ul>
 		<li v-for="(item,index) in scanlist" :key='index' class="petlist" v-if='item.scannerList!=null'>
 		<div class="petimg"><img :src="item.pet.portrait"/></div>
 		<div class="petmsg">
 		<p class="petname">{{item.pet.name}}</p>
 		<p class="pettime">扫码地点:{{item.scannerList.scannerPlace}}</p>
 		<p class="pettime">扫码时间:{{item.scannerList.time}}</p>
 		</div>
 	</li>
 	</ul>
	 </el-tab-pane>
...
import { listScanner } from "../../ClientServerApi/index.js";
export default {
  name: "mysearch",
  data() {
    return {
